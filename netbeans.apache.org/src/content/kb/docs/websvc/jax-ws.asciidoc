...
= Getting Started with Jakarta XML Web Services
:jbake-type: tutorial
...
link:https://jakarta.ee/specifications/xml-web-services/3.0/[Jakarta XML Web Services] and link:https://jakarta.ee/specifications/xml-web-services/3.0/apidocs/jakarta.xml.ws/module-summary.html[Jakarta API for XML Web Services (JAX-WS)], is an important part of the Jakarta EE platform. Jakarta XML Web Services (also referred to as JAX-WS) simplifies the task of developing web services using Java technology. It addresses some of the issues in former versions and JAX-RPC 1.1 by providing support for multiple protocols such as SOAP 1.1, SOAP 1.2, HTTP and by providing a facility for supporting additional protocols. JAX-WS uses Jakarta XML Binding 3.0 (formerly JAXB) for data binding and supports customizations to control generated service endpoint interfaces. 

This document demonstrates the basics of using Apache NetBeans to develop a Jakarta XML Web Service. After you create the web service, you write three different web service clients that use the web service over a network, which is called *_consuming_* a web service. The three clients are a Java class in a Java SE application, a servlet, and a JSP page in a web application. A more advanced tutorial focusing on clients is link:./client.html[+Developing JAX-WS Web Service Clients+].
...
|link:https://netbeans.apache.org/download/index.html[+Apache NetBeans+] |Apache NetBeans 12.0 or later

|link:https://openjdk.java.net/[+Java Development Kit (JDK)+] |JDK 8 or later
 

|Jakarta EE-compliant web or application server or servlet container including Eclipse Metro | Eclipse GlassFish or Payara Server Community Edition 
|===

*Note*: Both, GlassFish and Payara server can be installed upon request within Apache NetBeans IDE. Alternatively, you can visit the link:https://glassfish.java.net/download.html[+GlassFish server downloads page+], link:https://www.payara.fish/downloads/payara-platform-community-edition/#x[Payara Community downloads page+] or the link:https://tomee.apache.org/download.html[+Apache TomEE downloads page+], or pick another compliant server and set it up for use within NetBeans IDE.

*Important*: Jakarta EE projects require a compliant server.
